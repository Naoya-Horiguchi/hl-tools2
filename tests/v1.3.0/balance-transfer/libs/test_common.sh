...
instantiate_cc() {
    ...
    if [ "$endorsement_policy" ] ; then
        instantiateChaincode $token $channel "[\"$peer\"]" "$cc" golang "$cc_ver" "{\"endorsementPolicy\": \"${endorsement_policy}\", \"update\": $update, \"collectionFile\": \"${private:-null}\"}"
    else
        instantiateChaincode $token $channel "[\"$peer\"]" "$cc" golang "$cc_ver" "{\"endorsementPolicy\": null, \"update\": $update, \"collectionFile\": \"${private:-null}\"}"
    fi
